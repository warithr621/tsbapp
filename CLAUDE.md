...
- `routes/latex.js` — three endpoints:
  - `POST /api/upload-logo` — accepts a multipart upload (field `logo`, image only, 5 MB limit). Saves to `public/images/logo-upload.png`. If `saveAsDefault=true` in the body, also overwrites `public/images/logo.png`. Used as a pre-step before generate when the user picks a custom logo.
  - `POST /api/generate-latex` — accepts `{ round, counts, subjectOrder, writers, tournamentName, edition, useCustomLogo }`. `writers` is an optional map of subject → comma-separated writer names. `tournamentName` and `edition` are required strings that appear in the PDF title block and page footer. `useCustomLogo` (boolean) selects `public/images/logo-upload.png` instead of `public/images/logo.png` when copying the logo into `generated/`. Writes `.tex` files to `generated/`, then spawns two `pdflatex -interaction=nonstopmode` child processes in parallel (main packet + replacements). Uses `shell: true` for cross-platform PATH resolution. A module-level `compilationStatus` Map tracks `{ round, replacements }` each with `status: 'pending' | 'success' | 'error' | 'n/a'`. A `generationId` guard prevents stale pdflatex processes from overwriting a newer generation's status. Returns `{ success, hasReplacements }` immediately, before PDFs finish compiling.
...
2. User optionally fills writer names (comma-separated). User fills in the Packet Branding section: logo (default or upload-new with optional save-as-default), tournament name (prefilled "Texas Science Bowl Invitational"), and edition (required, e.g. "Spring 2026"). Then clicks Generate.
3. If a custom logo was selected, `view.js` first POSTs the file to `/api/upload-logo` (with `saveAsDefault` if checked). Then `view.js` POSTs `{ round, counts, subjectOrder, writers, tournamentName, edition, useCustomLogo }` to `/api/generate-latex`. The `.tex` download buttons appear immediately on response.
