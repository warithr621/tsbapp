...
const ROUND_NAMES = {
	'rr1': 'Round Robin 1', 'rr2': 'Round Robin 2', 'rr3': 'Round Robin 3',
	'rr4': 'Round Robin 4', 'rr5': 'Round Robin 5',
	'de1': 'Double Elimination 1', 'de2': 'Double Elimination 2', 'de3': 'Double Elimination 3',
	'de4': 'Double Elimination 4', 'de5': 'Double Elimination 5', 'de6': 'Double Elimination 6',
	'de7': 'Double Elimination 7',
...
function buildPreamble(round, title) {
...
async function generateLatexContent(questions, round) {
	// Group questions by subject and number
	const grouped = {};
	...
	let content = buildPreamble(round, ROUND_NAMES[round]);
...
async function generateReplacementsLatexContent(questions, round) {
	// Group replacement questions (all at questionNumber 6) by subject
	const grouped = {};
	for (const subject of SUBJECTS) {
	...
	let content = buildPreamble(round, `${ROUND_NAMES[round]} (Replacements)`);

	for (const subject of SUBJECTS) {
		const slot = grouped[subject];
		if (slot.Tossup || slot.Bonus) {
			if (slot.Tossup) content += questionTex(slot.Tossup, 1, true)  + '\n\n';
			if (slot.Bonus)  content += questionTex(slot.Bonus,  2, false) + '\n\n';
			content += '\\sep\n\n';
		}
	}
