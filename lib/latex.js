...
// Escapes LaTeX special characters in plain-text fields (branding, writer names).
// Single-pass replace avoids double-escaping (e.g. \textbackslash{} having its
// braces re-escaped by a subsequent {} rule). Does not handle math mode or \commands.
function escapePlainText(text) {
	if (!text) return '';
	const escaped = String(text).replace(/[\\&%$#_{}~^]/g, c => ({
		'\\': '\\textbackslash{}',
		'&':  '\\&',
		'%':  '\\%',
		'$':  '\\$',
	...
	return unicodeToLatex(escaped, 'text');
}
...
function buildPreamble(round, title, writers = {}, subjectOrder = [], branding = {}) {
	const tournamentName = escapePlainText(branding.tournamentName || 'Texas Science Bowl Invitational');
	const edition = escapePlainText(branding.edition || '');

	const activeSubjects = SUBJECTS.filter(s => subjectOrder.includes(s));
	const writerBlock = activeSubjects
		.map(s => `${SUBJECT_DISPLAY[s] || s}: ${escapePlainText(writers[s]) || '[Add Writers]'}`)
		.join('\n\n');
